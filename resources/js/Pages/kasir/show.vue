<template>
  <AuthenticatedLayout>
    <Head :title="`Detail Pasien - ${kunjungan?.nm_p || ''}`" />

    <div
      class="min-h-screen bg-cover bg-center flex items-center justify-center p-6"
      style="background-image: url('/images/bg-login.png')"
    >
      <div class="bg-white/70 backdrop-blur-sm rounded-xl p-6 shadow-lg w-full max-w-7xl mx-auto">
        <!-- Header -->
        <div class="flex flex-col lg:flex-row lg:justify-between lg:items-center mb-8 gap-4">
          ...
          <div class="flex flex-col sm:flex-row gap-3">
            <Link 
              :href="route('kasir.edit', kunjungan.id)" 
...
// Grand total as computed property
const grandTotal = computed(() => {
  return (
    calculateKonsulTotal() +
